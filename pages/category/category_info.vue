<template>
	<view>
		<u-navbar :placeholder=true :title="currTitle" :autoBack="true">
		</u-navbar>
		<view>
			<u-tabs :current="currLittleIndex" :list="list1" @click="clickTabs"></u-tabs>
		</view>
		...
		<view class="list_swiper_button_group">
			...
			<view class="goup_2">
				<u-row gutter="10">
					...
					<u-col span="3">
						<view><u-button throttleTime="1000" :disabled="currentPage<totalPage ? false:true" @click="next"
...
<script>
	...
	export default {
		...
		methods: {
			...
			clickTabs(item) {
				if (this.categoryId !== item["classId"] || this.childCategoryId !== item["id"]) {
					this.getCategoryImagesByJson(item["classId"], item["id"]);
					this.categoryId = item["classId"];
			...
			previous() {
				if (this.currentPage >= 1) {
					...
					this.getCategoryImagesByJson(this.categoryId, this.childCategoryId);
					this.swiperCurrent = 0;
				}
			...
			async getChildCategory(categoryId) {
				const res = await App._getRequest({
					url: '/getChildCategoryList?categoryId=' + categoryId,
					method: 'GET'
				});
				if (res.data["code"] === 200) {
					this.list1 = res.data["data"]["childCategory"];
				} else {
					uni.showToast({
						icon: "error",
						title: "服务器出现问题咯！",
			...
			async getCategoryImagesByJson(categoryId, childCategoryId) {
				const res = await App._getRequest({
					url: `/getCategoryImages?pageNumber=${this.currentPage}&categoryId=${categoryId}&childCategoryId=${childCategoryId}`,
					method: 'GET'
				});
				if (res.data["code"] === 200) {
		...
		onLoad(option) {
			...
			if (Object.keys(option).length != 0) {
				__this.currTitle = option.title;
				var index = option.index;
				__this.getChildCategory(option.classId).then(function() {
					console.log(Math.floor(__this.list1[index].picCount / 6));
					__this.totalPage = Math.floor(__this.list1[index].picCount / 6);
					__this.currLittleIndex = index;
					__this.categoryId = __this.list1[__this.currLittleIndex].classId;
					__this.childCategoryId = __this.list1[__this.currLittleIndex].id
					__this.getCategoryImagesByJson(__this.list1[__this.currLittleIndex].classId, __this.list1[
						__this.currLittleIndex].id).then(setTimeout(function() {
